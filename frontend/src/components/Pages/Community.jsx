          ...
          filteredPrompts.map((prompt) => (
            ...
            >
              <div className="flex flex-col gap-1">
                <span className="text-xs font-semibold text-gray-400 flex flex-wrap gap-2">
                  {(Array.isArray(prompt.tags)
                    ? prompt.tags
                    : (prompt.tags || "coding,ai,prompt").split(",")
                  ).map((tag, i) => (
              ...
              <div className="flex items-center flex-wrap gap-2">
                <button
                  onClick={() =>
                    handlePromptExport(prompt._id, "json", true, prompt.title)
                  }
                ...
                <button
                  onClick={() =>
                    handlePromptExport(prompt._id, "md", true, prompt.title)
                  }
                  className="h-10 px-3 bg-gray-100 hover:bg-gray-200 text-[#203A3E] rounded-xl text-xs font-bold transition flex items-center gap-1 cursor-pointer"
                >
                  <FileText size={14} /> MD
                ...
                <button
                  onClick={() =>
                    handlePromptExport(prompt._id, "pdf", true, prompt.title)
                  }
